#include "geoHyperbola.h"

#include "../sidePanel/sidePanel.h"

/// @brief Constructor for hyperbola using foci and point on it
/// @param parent DrawingCanvas on which the object is
/// @param name Name of the object
/// @param focusA First focus 
/// @param focusB Second focus
/// @param point Point on the hyperbola
GeoHyperbola::GeoHyperbola(DrawingCanvas *parent, const wxString &name, GeoPoint *focusA, GeoPoint *focusB, GeoPoint *point)
    : GeoConic(parent, name, HYPERBOLA) {
    ...
    this->definition = HYPERBOLA_BY_FOCI_AND_POINT;
...
void GeoHyperbola::ReloadSelf() {
    switch (definition) {
        case BY_FOCI_AND_POINT:
            ...
            a = fabs((pt.GetDistance(focusA) - pt.GetDistance(focusB)))/2;

            double lin_ecc = center.GetDistance(focusA);
            b = sqrt(lin_ecc*lin_ecc - a*a);

            // Rotate the hyperbola such that its axis are parallel to x,y-axis
...
void GeoHyperbola::CreateCopy(std::unordered_map<GeoObject *, GeoObject *> &copiedPtrs)
{
    GeoHyperbola* copy = new GeoHyperbola();
...
ListItem GeoHyperbola::GetListItem()
{
    switch (definition) {
...
wxString GeoHyperbola::DefToString(const HyperbolaDefinition &def) {
    switch (def) {
        case HYPERBOLA_BY_FOCI_AND_POINT:
            return "Hyperbola";
        default:
